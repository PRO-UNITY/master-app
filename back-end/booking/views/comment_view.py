...
from booking.serializers.comment_serializer import CommentsSerliazer, CommentSerliazer
...
class BookingsView(APIView, Pagination):
    ...
    serializer_class = CommentsSerliazer

    def get(self, request):
        queryset = Comments.objects.filter(user=request.user)
    ...
    @swagger_auto_schema(request_body=CommentSerliazer)
    def post(self, request):
        valid_fields = {"title", "master",}
        unexpected_fields = check_required_key(request, valid_fields)
        if unexpected_fields:
            return bad_request_response(f"Unexpected fields: {', '.join(unexpected_fields)}")
        serializers = CommentSerliazer(data=request.data, context={'user': request.user})
